...
do
  echo -e "\n\nProcessing $f file..."
  mv $f temp.json
  jq -r --arg dburl "$DATABASE_URL" --arg dbuser "$POSTGRES_USER" --arg dbpassword "$SUPERSET_POSTGRES_PASSWORD" '.config["connection.url"] |= $dburl | .config["connection.user"] |= $dbuser | .config["connection.password"] |= $dbpassword' temp.json > $f
  rm temp.json
  curl -s -X POST -H "Accept:application/json" -H  "Content-Type:application/json" http://connect:8083/connectors/ -d @$f
done
